...
impl Tags {
    ...
    pub fn get_tagged_connections(&self, key: &str, value: &str) -> Option<Vec<String>> {
...
mod tests {
    ...
    fn general_tests() {
        ...
        let connections_by_asset = tags.get_tagged_connections("asset", "BTCUSD").unwrap();

        assert_eq!(connections_by_asset.len(), 2);

        let connections_by_id = tags.get_tagged_connections("userId", "1").unwrap();
    ...
    fn remove_connection() {
        ...
        let connections_by_id = tags.get_tagged_connections("userId", "3").unwrap();
        assert_eq!(connections_by_id.len(), 1);

        assert!(tags.get_tagged_connections("userId", "2").is_none());
    }
    ...
    fn test_update_tag() {
        ...
        let connections_by_asset = tags.get_tagged_connections("asset", "ETHUSD").unwrap();
        ...
        let connections_by_id = tags.get_tagged_connections("userId", "2").unwrap();
        assert_eq!(connections_by_id.len(), 1);
    }
}
